...
import OlaMapComponent from '../components/map/OlaMapComponent'
import { useRouteLocation, useRouteCoordinates } from '../api/useRouteLocation'

const API_BASE_URL = import.meta.env.VITE_API_BASE_URL
const OLA_MAP_API_KEY = import.meta.env.VITE_OLA_MAP_API_KEY

interface OlaMapPageProps {
  id: string
}

function OlaMapPage({ id }: OlaMapPageProps) {
  const {
    data: locationData,
    ...
    <Box>
      ...
      >
        <Typography variant="h5" sx={{ color: '#fff' }}>
          Route Location View (Ola Maps) - Tour #{id}
        </Typography>
      </Toolbar>
      ...
      {!OLA_MAP_API_KEY && (
        <Alert severity="warning" sx={{ mb: 2 }}>
          Ola Maps API key not configured. Set VITE_OLA_MAP_API_KEY in your .env file.
        </Alert>
      )}
